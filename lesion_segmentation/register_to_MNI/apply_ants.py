...
def create_ants_registration_pipeline(name='ants_registration'):
    ...
    def reverse_list(transforms):
        return transforms[::-1]
        
    rev_transforms=Node(util.Function(input_names=['transforms'],
                            output_names=['rev_transforms'],
                            function = reverse_list), name="rev_transforms")
    ...
    ants_registration.connect([
                          ...
                          (inputnode, rev_transforms,  [('transforms_anat2MNI', 'transforms')]),
                          (rev_transforms, ants_reg,  [('rev_transforms', 'transforms')]),
